...
### 사내 검증에서 드러난 것: `model`은 아무 일도 안 한다

실제 게이트웨이에 붙여 보고 나서야 알았다. `llm check`는 전부 초록인데
게이트웨이는 `openai/gpt-oss-120b`로 응답했다 — config에는 `gauss-o-flash`라고
적혀 있는데도.
...
| 실험 | 결과 | 뜻 |
|---|---|---|
| `GET /models` | **405 Method not allowed** | 게이트웨이가 **모델 목록을 열지 않는다** |
| `model_id`를 다른 값으로 | **되던 호출도 전부 실패** | `X-LLM-MODEL-ID`가 **라우팅한다** |
| `model`을 없는 이름으로 | **정상 응답** | body의 `model`은 **검증도 안 된다** |

```
X-LLM-MODEL-ID: 339             ← 이것이 모델을 고른다 (= openai/gpt-oss-120b)
body {"model": "gauss-o-flash"}  ← 게이트웨이가 무시한다. 사람이 읽는 이름표일 뿐
```

**`gauss-o-flash`라고 적은 것이 처음부터 아무 일도 하지 않았다.** 오타를 냈어도
똑같이 돌았을 것이다. 이건 이 리포가 계속 경계해 온 형태다 — *사람이 적은 제약이
아무 효과 없이 통과하는 것*. config를 읽는 사람은 "이 시스템은 gauss-o-flash로
판정한다"고 믿는데 사실이 아니다.

### 그래서 확인한 것을 박제한다

`GET /models`가 405이므로 **"339번이 무슨 모델인가"를 런타임에 알아낼 방법이
없다.** 확인할 수 있는 유일한 경로가 응답에 실려 오는 이름이다.

```json
"llm": {
  "model": "gauss-o-flash",                        // 이름표(body에 실리지만 무시된다)
  "model_id": "339",                               // 진짜 선택자(헤더)
  "expect_reported_model": "openai/gpt-oss-120b"   // 사람이 확인해 적는다
}
```

| 상태 | 결과 |
|---|---|
| 박제 없음 + 이름 일치 | 조용히 통과 (body model을 존중하는 보통의 게이트웨이) |
| 박제 없음 + 불일치 | **적어야 할 줄을 알려주며** 실패 |
| 박제 있음 + 일치 | 통과 |
| 박제 있음 + 불일치 | **"게이트웨이가 모델을 바꿨다"** — 시끄럽게 실패 |
| 게이트웨이가 이름을 안 실어 줌 | 통과 (확인할 방법이 없다) |

마지막 줄이 중요하다. **확인할 수 없는 것을 실패로 만들면 그 신호는 곧 무시된다.**

그리고 **고칠 수 없는 빨간 테스트는 결국 꺼진다.** 꺼진 테스트는 없는 것보다
나쁘다 — 있다고 믿게 되니까. 그래서 "요청한 이름과 다르다"를 영구 실패로 두지
않고, 사람이 한 번 확인해 적으면 통과하되 **그 뒤로 바뀌면 잡는** 형태로 만들었다.

```console
$ python -m src llm check          # 박제 전
  ⚠  요청한 이름(gauss-o-flash)과 응답한 이름(openai/gpt-oss-120b)이 다르다.
     ... config의 llm에 다음을 적어라:
         "expect_reported_model": "openai/gpt-oss-120b"

$ python -m src llm check          # 박제 후
  모델     ✅ openai/gpt-oss-120b (게이트웨이가 응답에 실어 준 이름)

$ python -m src llm check          # 게이트웨이가 나중에 바꾸면
  ⚠  게이트웨이가 모델을 바꿨다 — config는 openai/gpt-oss-120b를 박제했는데
     meta/llama-3-70b로 응답했다. 의도한 변경이면 config를 갱신하고, 아니면 사내에 확인하라
```

**보고서에는 실제로 답한 이름을 적는다**(12단계). "gauss-o-flash가 판정했다"고
쓰인 보고서는 거짓이다.
...
| `기대한_모델이_실제로_답한다` | 박제한 모델이 아닌 것이 답하고 있다 — 답은 오므로 다른 방법으로는 모른다 |
