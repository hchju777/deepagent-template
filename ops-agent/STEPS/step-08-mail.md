...
지금은 **HTTP 200이면 보냈다고 본다.** Agent가 200 안에서 실패를 말하는 방식을
모르기 때문이다. 그래서 응답을 **통째로 남긴다** — 우리가 삼키지 않는 것이 최선이다.

```json
{"status": "sent", "response": {"session_id": "...", "outputs": [...]}}
```

실제 응답을 한 번 보고 나면 판정을 좁힐 수 있다. 그전에 "성공했다"고 단정하면
**메일이 안 갔는데 갔다고 기록되는** 상태가 만들어지고, 그게 제일 나쁘다.

멱등(같은 보고서를 두 번 안 보내기)도 아직 없다. 케이스 저장소가 생기는 6단계
이후에 "보낼 의도를 먼저 기록하고, 보낸 뒤 표시한다"는 2단계 레저로 붙인다.
...
그리고 **Agent 응답을 붙여 주시면** 성공 판정을 좁히겠습니다 — `-s`로 돌리면
`Agent 응답:` 줄에 찍힙니다.
...
tests/infrastructure/test_mail_agent.py  20   무력화, **진짜 HTTP 주입 시도**, 실패 경로
