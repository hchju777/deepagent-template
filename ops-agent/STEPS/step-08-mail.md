...
사내에서 돌려 확인한 세 가지다. **이 셋이 다음 단계의 설계를 결정한다.**

| 확인 | 결과 | 무엇이 결정되나 |
|---|---|---|
| 주입 방어 | ✅ 제목·수신자가 그대로였다 | 위 ②가 **실제 Agent 파서에서도 통한다** |
| 수신자 여러 명 | ✅ 전원 수신 | `to_email : a@x, b@y` 쉼표 구분이 통한다 |
| 본문 렌더링 | ✅ **렌더링돼서 온다. HTML도 된다** | **보고서를 HTML로 쓴다** |

### HTML이 된다는 것의 무게

평문이었다면 보고서에 표를 쓸 수 없고, 수십 개 지표를 줄글로 늘어놓아야 했다.
HTML이 되면 **표·강조·색**을 쓸 수 있고, 그러면 "커버리지를 숫자보다 먼저"
같은 원칙을 **시각적으로도** 지킬 수 있다 — 읽는 사람이 첫 화면에서 결론을
내리기 때문에 순서와 강조가 곧 방어선이다.

그래서 리포트 렌더러는 **마크다운과 HTML 두 벌**을 같은 데이터에서 만든다.
파일로 남길 때는 마크다운(diff가 읽힌다), 메일로 보낼 때는 HTML.
