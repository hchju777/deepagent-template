...
## 함정 ⑦: 작업 스케줄러로 아침 리포트를 걸 때

평일 08:00에 `report run`을 돌리는 것이 9f단계의 목적이다. Windows 작업 스케줄러에서
세 가지가 실제로 걸린다.

```
프로그램:  C:\ops-agent\.venv\Scripts\python.exe
인수:      -m src report run
시작 위치: C:\ops-agent          ← 비워 두면 config를 못 찾는다
```

1. **시작 위치(작업 디렉터리)를 반드시 적는다.** 비워 두면 `C:\Windows\System32`에서
   돌아 `config/`를 못 찾는다. 증상은 "손으로는 되는데 스케줄로는 안 된다"다.
2. **로그로 넘기면 인코딩을 탄다** — 함정 ②와 같은 문제다. 작업의 환경에
   `PYTHONUTF8=1`을 주거나, 리다이렉트 대신 콘솔 출력을 그대로 둔다.
3. **종료 코드를 감시하도록 건다.** `report run`은 실패를 종료 코드로만 말한다
   (파일 실패·발송 실패·못 읽은 법인 → 1). 스케줄러가 그것을 사람에게 알리지 않으면
   리포트가 몇 주 안 나가도 아무도 모른다 — 그게 이 단계가 막으려는 실패다.

`"사용자가 로그온했을 때만 실행"`으로 두면 아침에 아무도 로그온하지 않은 서버에서는
안 돈다. 계정 비밀번호를 저장하고 "사용자가 로그온하지 않아도 실행"으로 둬야 한다.
