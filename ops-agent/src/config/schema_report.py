...
class Thresholds(StrictModel):
    ...
    # 이슈 표에서 **유형별로** 몇 건까지 보일 것인가(급증·신규·소멸·반복 각각).
    # 한 표 전체의 상한이 아니라 유형별인 이유: 한 유형이 수십 건이면 그것이 표를
    # 독차지해서 다른 유형이 한 줄도 못 들어온다.
    #
    # 기본값이 작은 이유: 법인이 늘면 이슈 행은 법인 수에 비례해 늘어난다(특히
    # 반복은 (GBM·법인·라인·항목) 단위라 제일 빠르게 불어난다). 100줄짜리 이슈
    # 표는 아무도 안 읽고, 안 읽히는 표는 이슈가 없는 것과 같다.
    # 생략된 건수는 리포트가 **반드시 말한다** — 조용히 자르면 거짓이 된다.
...
class CommentSpec(StrictModel):
    ...
    **읽고 고치기 쉬운 형태**여야 한다.
    ...
    """
    enabled: bool = False
    prompt_file: str = Field(default="prompts/alarm-daily.md", min_length=1)
