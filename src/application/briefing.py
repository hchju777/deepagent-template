...
def one_line(text):
    """개행을 접는다 — `[...]` 섹션 어휘를 쓰는 **모든 프롬프트**가 이것을 공유한다:
    ...
    현황·질문답변 로그·재작성 요청), 서브에이전트 도구 반환(`subagents.py`). 한 벌만
    있어야 한다 — 두 벌이 생기면 언젠가 한쪽만 고쳐진다(실제로 그랬다).

    **여러 줄이 정상인 내용은 애초에 날것으로 안 들어온다**: 증거 본문과 순찰 스냅샷은
    둘 다 `repr()`을 거치고 그것이 개행을 이스케이프한다(`nodes.py`의 `repr(body)[:160]`,
    `runner.py`의 `repr(result.data)[:2000]`). 그래서 남은 자리는 전부 "항목 하나가 한
    줄"이고 접기가 정확한 방어다. 그 `repr` 두 곳은 테스트가 계약으로 못박는다.
