...
def one_line(text):
    """개행을 접는다 — **사람·HTTP에서 들어온 문자열을 싣는 세 프롬프트**가 이것을 쓴다:
    브리핑(`build_briefing`·`render_rules`·`render_deployment`), 접수(`intake._prompt`),
    사이트 선택(`scope._prompt`). 셋 다 `[...]` 섹션 어휘를 쓰므로 한 벌만 있어야 한다 —
    두 벌이 생기면 언젠가 한쪽만 고쳐진다(실제로 그랬다).

    **덮지 않는 것**: 조사 그래프 내부 프롬프트에는 같은 벡터가 **실재한다** —
    `nodes.py`의 qa_log(사람 답변)·가설 문장·task.error, `llm_judge.py`의 스냅샷 텍스트.
    별개 항목이다(도구 결과는 여러 줄이 정상이라 접기만으로 될지 먼저 따져야 한다).
    `history._clean_line`은 evidence id 제거를 겸한 두 번째 접기 구현이다 — 브리핑의
    네 번째 생산자이고, 언젠가 이 함수 위에 얹어야 한다.
