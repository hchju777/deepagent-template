...
**이 함수를 쓰는 것은 CLI와 워커다** — `case resume`·`chat`(`src/__main__.py`)과
`worker.consume`이 답을 소비할 때. HTTP는 여기 안 온다: `api`는 실행자가 아니라
(규율 9의 형제) 라우트가 `intake_turn`이나 `submit_answer`를 직접 부르고, 조사 답변은
명령 채널에 실린 뒤 워커가 집어 갈 때 비로소 이 함수를 지난다.

그래서 이 분기는 **두 자리**에서 일어난다(CLI 둘, 워커 하나 — 호출부 셋). 각자 베끼면 언젠가
하나가 빠뜨리므로 함수는 하나로 둔다 — 케이스 종결 세 경로가 발행 배선에서 겪은
그것이다(규율 8).
"""
