...
계획 12 이전에는 접수가 끝난 뒤에야 `repo.new_case_id()`를 불렀다. CLI에서는
프로세스 하나가 처음부터 끝까지 서 있어 드러나지 않지만, HTTP에서는 셋이 함께
깨진다 — 첫 요청이 돌려줄 case_id가 없고, 접수 중 클라이언트가 끊기거나 서버가
재시작되면 문답이 통째로 사라지고, 그 사이의 상태를 담을 곳이 없다.
