...
| 키 | 타입 | 기본값 | 설명 |
|---|---|---|---|
| `access.subjects` | dict | `{}` | 주체 → 토큰(`{"alice": "${ALICE_TOKEN}"}`, `SecretStr`). `api`가 `Authorization: Bearer <토큰>`을 역조회해 주체를 얻는다. **비어 있으면 모든 요청이 익명**이고 `allow`가 비어 있을 때만 통과한다. 틀린 토큰은 익명이 아니라 401이다 — 익명으로 떨어뜨리면 `allow`가 빈 설치에서 아무 문자열이나 통과한다. 발급·회전·세션은 리버스 프록시의 몫 |
...
10. `mongo_find` 점검의 `params.filter`가 dict이고 연산자가 허용 목록 안인가, `params.sort`가 `[[필드, 1|-1]]` 모양인가, 정적 필터와 `resolve`의 키가 겹치지 않는가, 그리고 **필터도 해석기도 없는 전량 스캔이 아닌가**(의도한 전체 조회는 `resolve`의 `unfiltered`로 명시한다)
...
15. `access.subjects`의 토큰이 비어 있지 않은가 — `${ENV}` 미설정이 조용히 빈 토큰이 되면 그 주체는 `Bearer `만 보내면 누구나다
