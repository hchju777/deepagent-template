...
rank 1 = 가장 유력한 후보(판정에 `root_cause`가 있으면 그것 — 신뢰도는 판정의 것 —,
inconclusive처럼 없으면 첫 후보), 그 뒤가 나머지다. "결론이 있는가"는 `verdict.root_cause`로
본다. 소독은 컴포넌트 이름의 공백을 걷고 빈 이름을 버리며 `relation`을 300자로 자른다.

**MetricRollup** — 집계 지표 하나의 결과. validator가 정직성을 강제한다: 커버 수가
기대와 다르면 `complete=False`가 **강제**되고, 불완전은 사유를 요구하며, 커버가 0이면
값은 `None`(0이 아니다), 값이 없는 "완전"도 사유를 요구한다. 표본의 불완전을 AND로 접는
것(`fold_complete`)은 모델이 아니라 `run.py`가 한다 — 타입이 막는 것과 조립이 지키는 것을
섞어 읽지 마라. "30개 중 3개
누락인데 숫자만 내보내는" 상태를 표현할 수 없게 만드는 것이 목적이다.

**커버리지(집계)** — 어느 사이트가 이 집계에 기여했는가. `covered`/`missing`/`fallback`
셋이고 뒤 둘은 사유가 필수다. 리포트에서 **숫자보다 먼저** 렌더된다 — 순서 자체가
방어선이다(숫자를 먼저 읽고 결론을 내리므로).

**scenario_digest** — 시나리오의 `kind`·`concern`·`metrics`·`group_by`·`scope`를 접은
해시. `extract`/`reduce`를 바꾼 뒤 어제 숫자와 나란히 그리면 추세가 거짓이 되므로,
digest가 다르면 추세를 비교 불가 caveat으로 대체한다. 제목처럼 표현만 바꾼 변경은
digest를 흔들지 않는다.

**Ticker** — 경과 시간의 소스(`Callable[[], float]`, 단조). `Clock`과 **다른 양이다**:
`datetime.now()` 금지의 목적은 기록 시점의 재현·감사이고, 경과는 재현 불가능한 것이
정상이다. 둘을 한 포트로 섞으면 고정 시계 테스트에서 duration이 항상 0이 된다.

**tier 검색** — 과거 종결 케이스를 벡터 없이 찾는 결정론 사다리(계획 15).
①같은 지문 ②같은 대상·같은 사이트 ③같은 대상·다른 사이트 ④상류 대상. 낮은 tier로
한 번만 담고 K건(3)에서 멈춘다. **결과에도 렌더에도 evidence id가 없다** — 과거 id를
리드가 인용하면 verify의 인용 우주(state.evidence)를 그대로 통과한다.

**RootCauseLabel** — 사람이 되먹인 실제 원인(append-only, 케이스당 복수). `agreement`
4분류가 component 문자열 비교보다 중요하다(자유 문자열은 절대 일치하지 않는다).
`resolution`은 "에이전트가 틀렸다"와 "실은 안 고장났다"를 가른다. 캘리브레이션 계산은
라벨 30건 **그리고** 종결의 절반 초과까지 열리지 않는다.

**관측성 푸터** — 보고서 마지막 두 줄. 경과·라운드·도구 실패와 **못 잰 것의 이름**(토큰),
그리고 라벨 유입구. 안 잰 것을 0으로 적으면 나중에 분모가 거짓이 된다.

**Timeline** — 보고서 §5의 첫 항목. 저장된 이벤트 로그(6종)를 `ReportModel`이
seq 순으로 요약한 표다. "이벤트 로그 없음"(이 프로세스에 스토어가 없다), "이벤트
없음"(로그는 있는데 비었다), "이벤트 로그 읽기 실패"(스토어가 던졌다 — 발행은 계속된다)는
다른 말이다.
...
**명령 채널** — 프로세스 밖에서 온 사람의 답이 워커에 닿는 길(계획 13). 새 큐가
아니라 레코드의 필드 둘이다: `pending_answer`(답)와 `answer_key`(멱등 키). `api`가
싣고 데몬의 `requeue_job`이 집어 워커가 `answer_case`로 소비한다. 워커는 답을
**지운 뒤** 실행한다 — 지우기 전에 실행하면 실패 시 같은 답이 또 들어간다.
`answer_key`는 소비 뒤에도 남긴다 — 클라이언트 재시도가 답을 두 번 넣지 않게.
...
**stale_question** — 클라이언트가 답한 질문이 더 이상 현재 질문이 아니다(계획 17).
사람이 Q1을 읽고 답을 쓰는 사이 조사가 Q2로 파킹하면 그 답은 Q2의 답이 아니다. 대조는
답을 **싣는 그 한 동작 안에서** 한다 — 미리 비교하고 저장소에 넘기면 그 사이가 다시
창이다. 번호를 안 보내면 예전처럼 받는다(하위 호환).

**intake_done** — requeue의 문. 접수 중인 케이스도 `open`이라 이 표시가 없으면
데몬이 그것을 집어 대상 없이 조사한다(계획 12의 F1 경합이 그것이었다). 사람이
연 케이스는 `False`로 열리고 접수가 끝나거나 포기하면 `True`, 순찰이 연 케이스는
finding에서 대상이 오므로 처음부터 `True`다.

**requested_by** — 케이스를 요청한 주체. `app.json`의 `access.allow`가 비어 있지
않으면 접수 경계에서 `(주체, gbm, fct)` 허용 판정을 받는다. 읽기 전용이라고 폭발
반경이 작지 않다 — 케이스 하나는 그 법인의 Redis/Mongo/Kafka와 소스 저장소를 읽는
LLM 에이전트를 돌리라는 요청이다. 빈 테이블은 "제한 없음"이고, 선언이 있는데 주체가
없으면 거부한다.

**question_kind** — `awaiting_human`이 두 가지를 뜻하기 때문에 필요한 구별.
`"intake"`는 접수가 되물은 것, `"investigation"`은 그래프가 interrupt한 것이다.
답을 받은 쪽이 이 둘을 섞으면 아직 없는 스레드를 재개하려다 실패하고, 사람 눈에는
"답했는데 조사가 깨졌다"로 보인다. `None`은 계획 12 이전 레코드이고 그때는 그래프
파킹만 존재했다.

**사이트 해석(scope resolution)** — 웹 사용자는 `--gbm/--fct`를 주지 않으므로 증상
하나로 사이트를 정하는 단계. **후보는 코드가 registry에서 만들고 LLM은 그 안에서만
고른다** — 목록 밖 값은 확정이 아니라 미확정이다. 미확정이면 케이스를 만들지 않는다.
...
**`question_seq` / `answered_seq`** — "답이 어느 질문의 답인가"를 세는 짝. 파킹마다
`question_seq`가 오르고, 워커가 답을 가져가면 `answered_seq`를 그 값으로 맞춘다.
클라이언트가 본 번호를 실어 보내면 저장소가 **답을 싣는 그 한 동작 안에서** 대조하고,
다르면 `stale_question`(HTTP 409)이다. 안 보내면 예전 동작이다(하위 호환).

**`mongo_find`** — 특정 컬렉션에 특정 필터로 질의하는 프로브. `probe`로 **명시할 때만**
쓰인다(`mongo:` 접두사의 기본은 여전히 `mongo_recent`). 필터 연산자는 닫힌 허용 목록을
통과해야 하고, 그 판정 함수를 어댑터·스텁·기동 검증이 **공유한다**.

**`one_line`(개행 접기)** — `[...]` 섹션 어휘를 쓰는 모든 프롬프트가 공유하는 방어.
한 줄이 쪼개지면 그 조각이 다음 섹션 머리말처럼 보이고 위조된 블록이 진짜보다 **먼저**
온다. 여러 줄이 정상인 내용(증거 본문·순찰 스냅샷)은 개행을 이스케이프하는 표현을 거친다
(`evidence_summary`·`snapshot_text`·`json.dumps`).

**tier(이력 사다리)** — 과거 종결 케이스를 찾는 결정론 순서. ①같은 지문 ②같은 대상·같은
사이트 ③같은 대상·다른 사이트 ④상류 대상. 낮은 tier로 한 번만 담고 K=3에서 멈춘다.
**tier마다 저장소를 따로 질의한다** — 한 질의를 나눠 쓰면 절단이 분리보다 먼저 일어나
사다리가 뒤집힌다.

**캘리브레이션 게이트** — 종결 라벨 30건 **그리고** 종결의 절반 초과 전에는 어떤 정확도도
계산하지 않는다. 임계는 코드가 쥔다(config로 빼면 게이트가 협상 대상이 된다). 열려도 내는
것은 상관계수가 아니라 `confidence`별 적중이다 — 대상이 범주형이라 Pearson r은 범주 오류다.
