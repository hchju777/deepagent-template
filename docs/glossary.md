...
**MetricRollup** — 집계 지표 하나의 결과. validator 넷이 정직성을 강제한다: 누락이
있으면 `complete=False`가 **강제**되고, 불완전은 사유를 요구하며, 커버가 0이면 값은
`None`(0이 아니다), 표본에 불완전이 하나라도 있으면 전체가 불완전이다. "30개 중 3개
