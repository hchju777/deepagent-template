...
설계 부품이 실제로 어떻게 맞물리는지 보여주는 두 사건. **§5.5의 간판 시나리오 E2E의 씨앗**이기도 하다 — 구현 시 이 둘을 첫 시나리오로 스텁 위에 재현한다.

가정 토폴로지(두 사건 공통): `kafka edge.raw.{line}`(설비 실적·사이클 이벤트) → `twin-aggregator` → `mongo twin_state` → `twin-api` → `REST /oee`. aggregator는 분모로 Redis `plan:{line}:today`(당일 계획, `plan-sync`가 씀)를 참조. 설비 상태는 `equip-sync`가 kafka를 소비해 Redis `equip:{line}:status`에 캐시.

### A.1 룰 탐지 — "OEE 512%" (있을 수 없는 값)

1. **탐지 (rule, LLM 없음)**: 10분 주기 `api.oee_range`가 전 라인 `/oee` GET → line 7이 512%, 룰(0~100) 위반 → 응답을 스냅샷 `ev-1`로 박제, Finding 발행 → 지문 중복 없음 → 케이스 생성, 큐 enqueue.
2. **frame**: 브리핑(토폴로지 슬라이스, 룰, 유사 이력 "7월 OEE 300% — 0-나눗셈"). 가설: H1 계산 이상 / H2 원천 이상 / H3 서빙 이상. 계획: t1 mongo 조회, t2 kafka 샘플, t3 코드 추적(배포 hash로), t4 재계산 — **t4는 입력이 t2·t3의 증거 id라 select 게이트에 걸려 대기**.
3. **라운드 1** (t1·t2·t3 병렬): t1 → `twin_state`: oee=5.12, planned_time=75 (`ev-2`). t2 → kafka 실적 정상 (`ev-3`). t3 → 로직 명세: 분모는 Redis `plan:{line}:today`, **키 부재 시 마지막 캐시값 폴백** (`ev-4`, commit hash 포함).
4. **integrate #1**: mongo도 512% → H3 기각(ev-2). 원천 정상(ev-3) → H2 약화. ev-4에서 새 가설 H4(plan 키 문제), 태스크 t5(Redis plan 키 조회) 추가. t4는 입력 확보로 실행 가능.
5. **라운드 2**: t5 → `plan:7:today` **키 없음**(봉투 complete ✓, 옆 라인 키는 존재) (`ev-5`). t4 → 재계산: 정상 계획이면 87%, 실제 512%는 분모 75일 때 정확히 재현 — 불일치가 분모 하나로 국소화 (`ev-6`).
6. **integrate #2 → conclude**: plan-sync가 line 7 키를 못 썼고(ev-5), aggregator가 옛 값(주말 소량 계획)을 폴백으로 사용(ev-4)해 분모 축소 → OEE 폭등(ev-2·ev-6).
7. **verify**: 인용 증거 실재 ✓. 부정 증거("키 없음")의 봉투가 complete인지 확인 ✓ — SCAN이 상한에 잘린 결과였다면 여기서 기각.
8. **보고서**: 판정 stale_data / 원인 컴포넌트 plan-sync. 권고: ① 키 재생성(즉시) ② plan-sync 실패 원인 로그 확인 — **스코프 밖(LogReader 미도입) 명시, 사람 요망** ③ aggregator의 폴백을 명시적 미계산 처리로 개선 검토.

쿼리 분업의 예: t1에서 어댑터는 `mongo_find(collection, filter, limit)` 빌더만 노출(개발자), LLM은 빈칸만 채움(`twin_state`, `{line:7, ts≤T0}` — 컬렉션 이름은 토폴로지에서), guards가 상한 강제(config).

### A.2 LLM 탐지 — "멈춘 라인이 생산을 한다" (모든 값이 각자 정상)

1. **프로브 (결정론)**: `twin.consistency`(judge: llm, cron 8시/20시, sample 20)가 라인 20개의 (Redis 설비상태 + Mongo 트윈상태 최근 1h) 쌍을 떠서 즉시 박제(`snap-08a`). line 12: 상태 `STOP since 04:10`(유효 enum), 생산량 시간당 +60(정상 증가율) — **어느 값도 룰 위반이 아니고**, `kafka.lag` 룰도 통과(equip-sync 그룹의 **총** lag은 임계값 아래).
2. **예산 확인 (코드)**: `llm_budget` 잔여 확인. 소진이면 레저에 "미점검" 기록하고 끝.
3. **LLM 판정 (호출 1회)**: "쌍 20개 중 의미적 모순을 근거 id 인용으로 보고하라" → "line 12: STOP인데 같은 구간 생산량이 꾸준히 증가 — 둘 중 하나는 거짓" (근거: snap-08a).
4. **가드레일 (결정론)**: 인용 근거가 저장 스냅샷에 실재·일치하는지 검증 ✓. 표본에 없는 라인을 환각 인용했다면 여기서 기각 — 유령 케이스는 안 열린다.
5. **경계 강제**: 판정기의 일은 Finding까지. 원인 파기는 케이스 큐 너머 조사 엔진이.
6. **조사 (압축)**: 쟁점은 "상태가 거짓인가, 생산량이 거짓인가". 토폴로지상 진실의 뿌리는 kafka. data_prober → `edge.raw.12`에 사이클 이벤트 계속 흐름(라인은 돈다 — 상태가 거짓) / equip-sync 그룹 **파티션별** 조회 → 파티션 12만 04:10부터 오프셋 정지(총량 룰이 놓친 이유). code_tracer → 파티션별 컨슈머 태스크 구조 확인.
7. **판정**: stale_data / 원인 컴포넌트 equip-sync (파티션 12 소비 정지 → 상태 캐시 갱신 중단; since와 오프셋 정지 시각 일치). 권고: ① 재기동/파티션 재할당 ② 정지 원인 로그 확인(스코프 밖 명시) ③ **`kafka.lag` 룰에 파티션별 정체 검사 추가** — config `params` 수정만으로 가능.

권고 ③이 이 설계의 경제학이다: **LLM 판정기는 룰이 못 잡는 것을 잡는 의미망이고, 잡힌 패턴은 싼 룰로 환류된다.** 같은 유형의 다음 사건은 하루 두 번짜리 LLM 점검이 아니라 10분 주기 무료 룰이 잡는다.
