...
1. ~~**접수 `_save`의 TOCTOU**~~ — **계획 17이 갚았다**(`update_if`). — 같은 케이스에 동시에 오는 두 `/intake-answers`가
...
9. ~~**`POST /cases/{id}/answers`에 `question_seq` If-Match가 없다**~~ — **계획 17이 갚았다**
   (HTTP·CLI·chat 세 경로 전부. 조사 질문은 `resume_once`가 lease를 잡은 뒤, 접수 질문은
   `intake_turn`이 레코드를 읽은 직후에 대조한다.) — 클라이언트가 Q1을 보고
