...
3. **`calibration`은 라벨된 종결 케이스마다 저장소를 두 번 친다**(`snapshots.get`,
...
4. **`saw_report`는 케이스가 아니라 세어진 라벨의 성질이다** — 마지막 라벨 한 행에서
   `agreement`와 함께 뽑는다. "먼저 눈감고 달았다가 보고서 보고 정정"한 케이스는
   앵커링으로 세어진다. 앵커링을 케이스 단위로 물으려면 이력 전체를 봐야 한다.
...
7. **메트릭은 `investigation.duration_s` 하나뿐이다** — 토큰은 여전히 미측정이고
