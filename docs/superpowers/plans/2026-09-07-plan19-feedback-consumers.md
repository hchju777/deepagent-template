...
1. **캘리브레이션은 `confidence`만 가른다** — `concern`·`origin`·`verdict_type`별로도
   ...
   때문이다. 라벨이 쌓이면 그때.
2. **`saw_report`는 케이스가 아니라 세어진 라벨의 성질이다** — 마지막 라벨 한 행에서
   `agreement`와 함께 뽑으므로, "먼저 눈감고 달았다가 보고서 보고 정정"한 케이스는
   앵커링으로 세어진다. 표시만 하고 판단하지 않는다 — 앵커링 보정(보고서를 본 라벨에
   가중치를 다르게)은 통계적 근거가 필요하고 지금은 그 근거가 없다.
3. **분모 규칙이 하나 더 있다 — 저장소에서 사라진 케이스의 라벨은 안 센다.**
   스냅샷과 라벨은 retention보다 오래 살므로(`domain/snapshot.py`) 케이스가 이미
   purge된 라벨이 존재한다. 종결 목록과의 교집합이 그것을 거른다 — "스냅샷 없는 라벨"과는
   다른 규칙이고, 게이트의 분모(`label_stats`)와 같은 모집단을 쓰려는 것이 이유다.
4. **`calibration`은 라벨된 종결 케이스마다 저장소를 두 번 친다**(`snapshots.get`,
   `labels.list_for`) — 게이트가 30건에서 열리므로 지금은 60왕복이고 사람이 손으로 치는
   명령이라 문제가 아니다. `list_by_status("closed")`도 두 번 돈다(`label_stats`에서 한 번,
   본체에서 한 번). 라벨이 수천 건이 되면 배치 조회가 필요하다.
5. **여러 사람이 같은 케이스를 다르게 라벨하면 마지막이 이긴다** — 합의가 아니라
   최신성이다. `labeled_by`가 이미 남아 있으니 필요해지면 그때 가른다.
6. **`"미상"` 버킷은 서열 축의 값이 아니다** — 표에서 맨 끝에 두지만, "미상이 low보다
   잘 맞네" 같은 무의미한 비교를 유도할 수 있다. 버리면 생존 편향이라 남기는 쪽을 골랐다.
7. **`patrol status`의 지표는 최근 200건 창이다** — 창이 꽉 차면 출력이 그렇게 말한다.
   정확히 200건인지 초과인지는 `limit=201`로 읽어야 확정되는데, 한 행 더 읽는 값어치가
   있는지는 판단의 문제라 지금은 보수적인 문구를 쓴다.
8. **메트릭은 `investigation.duration_s` 하나뿐이다** — 토큰은 여전히 미측정이고
