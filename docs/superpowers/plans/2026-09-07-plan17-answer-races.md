...
2. **`stale_question`은 답을 보관하지 않는다** — 사람이 쓴 답이 버려진다. 큐잉(늦은 답을
   ...
   문제의 원인이다.
3. **접수 CAS의 술어는 `updated_at` 하나다** — 같은 밀리초에 두 저장이 일어나면(고정 시계
   테스트) 둘 다 이길 수 있다. 프로덕션은 실시계라 도달 불가지만 엄밀히는 단조 카운터가 옳다.
4. **`_SAVED_FIELDS`가 접수 소유 필드의 유일한 목록이다** — 접수가 새 필드를 쓰게 되면
   여기 더해야 하고, 안 더하면 그 필드가 조용히 저장되지 않는다.
5. **CLI의 대조는 lease 검사 뒤다** — lease가 잡혀 있으면 번호가 틀려도 "데몬이 실행 중"이
   먼저 나온다. 사람에게 더 유용한 안내라 의도한 순서다.
