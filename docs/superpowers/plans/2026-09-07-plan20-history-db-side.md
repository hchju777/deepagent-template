...
6. **정렬 테스트는 삽입 순서와 시간 순서를 어긋나게 둬야 한다** — 같게 두면 동점 키가
   우연히 정답을 내서, 정렬 키를 무력화하는 변조가 통과한다. 이 계획에서 같은 함정을
   **네 번** 밟았다(검증 리뷰가 세 번, 동점 키를 새로 더한 뒤 한 번 더). 시각 정렬을
   테스트할 때 제일 먼저 확인할 것.
7. **`list_by_status`는 여전히 전량 하이드레이션이다** — `label_stats`·`calibration`이
