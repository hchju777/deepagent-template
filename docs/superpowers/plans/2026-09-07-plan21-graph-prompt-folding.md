...
6. **성질 테스트로 못 보는 갈라짐은 배선으로 단정한다** — 증거 요약의 두 생산자는 실전
   본문이 컨테이너라 `str`과 `repr`이 같아서, 한쪽만 되돌린 변조가 값 비교를 통과한다.
   그건 성질이 아니라 배선이므로 `inspect.getsource`로 단정했다(이 리포가 `patrol run`의
   시나리오 배선에 쓰는 관용구와 같다). **성질을 볼 수 있으면 성질로, 못 보면 배선으로** —
   구현 글자를 성질인 척 단정하는 것만 피하면 된다.
