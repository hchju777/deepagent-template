...
8. **이 계획에서 `inspect.getsource` 단정을 세 번 썼고 세 번 다 성질로 대체됐다** —
   두 생산자의 갈라짐(픽스처를 문자열 본문으로), 서브에이전트 `goal`(모델이 받은 메시지를
   가로채서), 판정 스냅샷(표현에 이름을 줘서). 이 리포에서 `getsource`가 정당한 자리는
   **"어느 함수를 부르는가"라는 배선 사실**뿐이다(`patrol run`의 시나리오, 발행 배선).
   구현 표현을 지키는 데 쓰면 등가 리팩터를 오탐하고 주석 한 줄에 우회된다.
9. **개행 접기와 길이 상한은 둘 다 "한 벌"이어야 한다** — `MAX_SNAPSHOT_CHARS`는 만드는
   쪽(`runner.snapshot_text`)과 렌더하는 쪽(`llm_judge._build_prompt`)이 리터럴을 각자
   적고 있었다. 상수를 되읽는 테스트로는 그 갈라짐을 못 본다(양쪽이 같은 상수를 읽으니
   항상 맞다) — 상수를 **바꿔 놓고** 실제 프롬프트 폭을 봐야 한다.
