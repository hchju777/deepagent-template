...
1. **그것이 옳은 분류이지 편의가 아니다.** 지금 존재하는 rule 4종은 전부 파이프라인 신호를 판정한다. 기존 점검을 전부 `system`으로 두는 것은 "생각을 건너뛴 결과"가 아니라 **맞는 답**이다.
2. **비용이 잘못된 자리에 든다.** 필수로 만들면 `CheckConfig` 구성 지점 ~100곳(테스트 60, 문서 40)을 고쳐야 하는데, 그중 concern이 의미 있는 곳은 거의 없다. 테스트 픽스처마다 라우팅과 무관한 필드를 적게 만드는 것은 신호가 아니라 소음이다.

대신 **오분류가 조용하지 않게** 만든다 — 보고서 헤더·메일 제목·`patrol status`에 concern을 노출한다. 잘못 분류된 점검은 첫 finding에서 사람 눈에 띈다. 이 판단이 틀렸다면(운영자가 실제로 오분류한다면) 그때 필수로 올린다 — 그 시점엔 근거가 생긴다.
