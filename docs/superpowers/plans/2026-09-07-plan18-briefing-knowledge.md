...
6. **브리핑의 모든 줄은 `_one_line`을 지난다** — 렌더러 두 개만 접고 `build_briefing`
   자신의 세 자리(`[증상]`·`[관련 서비스]`·파생 사슬)를 안 접으면 한 위협 모델에 대해
   반만 막은 것이다. 같은 서비스 이름이 한 줄 위에서는 접히고 아래에서는 안 접히는
   상태였다(검증 리뷰 I1). **브리핑에 새 줄을 더할 때 이 함수를 지나게 하라.**
7. **`docs_text` 부재 테스트는 그 이름만 지킨다** — 다른 이름으로 같은 섹션을 되살리면
   통과한다. 되돌림 방지에는 충분하고, "생산자 없는 섹션을 두지 않는다"는 성질은 사람이
   리뷰에서 본다(검증 리뷰 F6).
8. **배포 매핑은 여전히 코드 어댑터에 안 간다** — `build_adapters`는 `deployment`를 안 받고,
