...
11. **집계 지표도 점검과 같은 기동 검증을 받는다**(머지 직후 추가) — `MetricSpec`이
    프로브에 그대로 실리므로 `mongo_find` 지표의 필터·정렬·키 겹침을 boot이 본다.
    `sample`의 하한과 `params.filter` 겹침도 `CheckConfig`와 같아졌다. 새 프로브가
    생기면 **두 경로(점검·지표) 모두**에 검증을 붙여야 한다.
