...
11. **집계 지표의 해석기 검증은 점검과 같은 함수를 쓴다**(머지 직후 추가) — `MetricSpec`이
    프로브에 그대로 실리므로 `mongo_find` 지표의 필터·정렬·키 겹침을 boot이 보고,
    해석기 검증은 `_resolver_problems` 하나를 두 경로가 공유한다. 처음엔 지표 쪽에
    `mongo_find` 검증만 붙였는데 검증 리뷰가 **GET 강제를 포함한 5건이 여전히 점검에만
    돈다**는 것을 실측으로 잡았다 — 그중 GET 강제는 등재된 POST 항목을 가리키는 해석기가
    실제로 POST를 내보내는 경로였다. 각자 베끼면 하나가 빠뜨린다는 것이 요점이므로,
    새 검증을 더할 때는 **그 공유 함수에** 넣어라.
    `sample`의 하한과 `params.filter` 겹침은 지표 쪽이 **더 엄격하다** —
    `CheckConfig`의 겹침 검증자는 아직 `body`만 본다.
