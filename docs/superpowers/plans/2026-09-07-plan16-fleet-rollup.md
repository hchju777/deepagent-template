...
1. **축 분해는 `group_by == ["gbm"]`만 지원한다** — 다른 축(법인·라인)을 늘리려면
   `_groups`를 일반화해야 하고, 그때 축 조합의 카디널리티 상한을 코드가 쥐어야 한다.
2. **`required=False` 지표가 아직 아무 데서도 다르게 취급되지 않는다** — 스키마에는
   있고 `_rollup`이 안 본다. 선택 지표의 누락이 커버리지에서 빠져야 한다.
3. **`sample` 상한이 프로브에 전달만 되고 집계 쪽 상한은 없다** — 사이트 하나가 100만
   행을 돌려주면 `extract`가 그대로 돈다. 카디널리티 정직성(방향 문서 §110)은
   "몇 개 중 몇 개를 봤나"를 리포트에 적는 것까지 가야 완성된다.
4. **retention이 `fleet_runs`를 안 걷는다** — `DigestStorePort.prune_before`는 있는데
   `sweep_retention`이 호출하지 않는다. 추세는 최근 것만 쓰므로 knob이 필요하다.
5. **집계 메일의 제목·본문 관례는 케이스 보고서와 다르다** — 제목이 `[제목] 집계
   리포트` 한 형태뿐이고 수신자는 `concern`만 따른다. `send_id`는 창 시각을 포함해
   실행마다 유일하다(집행 중 발견해 고쳤다 — 이름만 쓰면 둘째 실행이 중복 억제된다).
6. **`api`가 시나리오 선언 자체를 안 낸다** — `GET /digests/{scenario}`는 실행 기록만
   준다. 웹 UI가 목록을 그리려면 `GET /scenarios`가 필요하다.
