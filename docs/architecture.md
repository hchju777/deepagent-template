...
- **브리핑 재료(`src/application/briefing.py`)** — 스펙 §3.6은 다섯을 열거하지만
  **넷만 실재한다**: 토폴로지 슬라이스, 적용 룰, 유사 이력, 배포 버전. **자유 문서는
  ...
  슬라이스 안일 때만 실린다. **브리핑의 모든 줄은 개행을 접는다**(`_one_line`) —
  한 줄이 쪼개지면 그 조각이 다음 섹션 머리말처럼 보이고, 위조된 `[적용 룰]`이
  진짜보다 먼저 온다. 가장 강한 벡터는 `[증상]`이다(`Finding.summary`가 그대로
  들어오고, `judge="llm"` 점검에서 그것은 LLM이 쓴 자유 문자열이다). 배포는 **"없음"을 절대 안 쓴다** — 매핑 자체가 없으면
