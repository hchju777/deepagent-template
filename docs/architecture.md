...
- **이력 tier 검색(`src/application/history.py`)** — 벡터 없이 결정론 사다리로 과거
  ...
  ④상류 대상. 낮은 tier로 한 번만, K=3
  (**tier마다 저장소를 따로 질의한다** — 한 질의를 나눠 쓰면 절단이 분리보다 먼저
  일어나 사다리가 뒤집힌다: 같은 locator의 최신 20건이 전부 다른 사이트면 더 강한
  tier 2가 조용히 0건이 되고 K를 tier 3이 채운다. 계획 22가 갚았고, 그전 tier
  테스트가 전부 상한보다 작은 픽스처를 써서 못 봤다). K를 채우면 다음 tier는
  질의조차 안 한다 — 제너레이터라 다음 항목을 요청하는 순간 질의가 돈다, `degraded`·요약 없는 케이스 제외(워커 실패의
  ...
  **정렬과 절단은 저장소가 한다**(계획 20): Mongo는 집계로 `$sort`+`$limit`을 걸어 10건을
...
- **브리핑 재료(`src/application/briefing.py`)** — 스펙 §3.6은 다섯을 열거하지만
  **넷만 실재한다**: 토폴로지 슬라이스, 적용 룰, 유사 이력, 배포 버전. **자유 문서는
  ...
  접는다**(`one_line` — 브리핑·접수·사이트 선택·리드의 frame/integrate/conclude(재시도 프롬프트의 검증 오류 포함)·서브에이전트 도구 반환과 태스크 goal·순찰 LLM 판정. `[유사 이력]`만 `history._clean_line`이 접는다 — evidence id 제거를 겸해서 갈라진 두 번째 구현이다) — 한 줄이 쪼개지면 그 조각이 다음 섹션 머리말처럼 보이고,
