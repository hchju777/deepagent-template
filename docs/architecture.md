...
- **접수 저장의 CAS** — `intake._save`가 **턴이 시작할 때 읽은** `updated_at`을 술어로
  걸어 조건부로 쓴다(`update_if_unchanged`). 재읽기 값으로 걸면 창이 좁아질 뿐 닫히지
  않는다: 두 턴이 각자 읽고 각자 LLM을 돌린 뒤 각자 재읽고 쓰면 둘 다 이긴다. 되묻기
  경로(`_park`)도 같은 술어를 탄다.
