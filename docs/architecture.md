...
- **브리핑 재료(`src/application/briefing.py`)** — 스펙 §3.6은 다섯을 열거하지만
  **넷만 실재한다**: 토폴로지 슬라이스, 적용 룰, 유사 이력, 배포 버전. **자유 문서는
  ...
  슬라이스 안일 때만 실린다. **사람·HTTP에서 온 문자열을 싣는 세 프롬프트가
  개행을 접는다**(`one_line` — 브리핑·접수·사이트 선택) — 한 줄이 쪼개지면 그 조각이 다음 섹션 머리말처럼 보이고,
  위조된 `[적용 룰]`이 진짜보다 먼저 온다. 고정 문자열과 `Literal`·`datetime` 필드는
  값 자체가 개행을 못 담아 지나지 않는다. 조사 그래프 내부 프롬프트는 아직 감사되지
  않았다(`one_line` docstring). 가장 강한 벡터는 `[증상]`이다
