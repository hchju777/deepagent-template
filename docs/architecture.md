...
- **이력 tier 검색(`src/application/history.py`)** — 벡터 없이 결정론 사다리로 과거
  ...
  **tier마다 저장소를 따로 질의한다**(계획 22) — 한 질의를 나눠 쓰면 절단이 분리보다
  먼저 일어나 사다리가 뒤집힌다: 같은 locator의 최신 20건이 전부 다른 사이트면 더 강한
  tier 2가 조용히 0건이 되고 K를 tier 3이 채운다. 그전 tier 테스트가 전부 상한보다 작은
  픽스처를 써서 여덟 라운드 동안 안 보였다 — **이력 테스트의 픽스처는 상한보다 커야 한다.**
  K를 채우면 다음 tier는 질의조차 안 한다(제너레이터라 다음 항목을 요청하는 순간 질의가
  돈다).
